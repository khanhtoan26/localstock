...
status: Phase 25 Wave 5 partial — 25-07 landed (DQ-02 Tier 2 dispatcher + DQ-03 promotion runbook; ROADMAP SC #4 ✅ closed). 25-08 (freshness probe) remains.
stopped_at: Completed 25-07-PLAN.md (Wave 5 — DQ-02 evaluate_tier2 shadow-by-default dispatcher + DQ-03 promotion runbook; SC #4 ✅)
last_updated: "2026-04-29T07:55:00.000Z"
last_activity: 2026-04-29 — 25-07 complete; Tier 2 advisory dispatcher landed. evaluate_tier2(rule, df, predicate, *, symbol) shadow-by-default with per-rule env-flag mode lookup (DQ_TIER2_<RULE>_MODE → dq_default_tier2_mode → 'shadow'). Tier2Violation only raised in enforce mode → caught by 25-06 per-symbol try/except. Predicates: predicate_rsi_anomaly (RSI > 99.5), predicate_gap_30pct (close-to-close > 30%), predicate_missing_rows_20pct (> 20% expected sessions). Wired into AnalysisService.analyze_technical_single after compute_indicators with predicate-bug isolation (logs dq.tier2.predicate_error, never breaks analysis). metrics.iter_tracked_collectors WeakSet pattern fixes multi-registry test contract. docs/runbook/dq-tier2-promotion.md (117 lines): Promotion Criteria, Procedure, Rollback, Per-Rule Status Table — closes DQ-03 deliverable. test_runbooks.py upgraded from existence-only to content-required. New SC #4 verbatim test test_sc4_tier2_emits_metric_no_block. 6/6 dispatch tests + 1/1 runbook test GREEN; 83/83 in test_services + test_dq + test_docs (no regressions). 3 commits with Copilot trailer. ROADMAP SC #4 verbatim ✅ closed.
progress:
  total_phases: 8
  completed_phases: 3
  total_plans: 24
...
**Current focus:** v1.5 Performance & Data Quality — Phase 25 Wave 5 partial: 25-07 complete (DQ-02 + DQ-03; SC #4 ✅). 25-08 (DQ-07 freshness probe) remains.

## Current Position

Phase: 25 — Data Quality (🚧 in progress; Wave 5 partial)
Plan: 07 (complete) — DQ-02 Tier 2 dispatcher + DQ-03 promotion runbook landed; ROADMAP SC #4 verbatim ✅ closed.
Status: 25-07 complete — `evaluate_tier2(rule, df, predicate, *, symbol)` shadow-by-default dispatcher with per-rule mode resolution (`get_tier2_mode(rule_name)` reads `Settings.dq_tier2_<rule>_mode` → `Settings.dq_default_tier2_mode` → hard fallback `'shadow'`). Tier label: `'advisory'` in shadow, `'strict'` in enforce. Always emits `localstock_dq_violations_total{rule, tier}` + structured `dq_warn` log; raises `Tier2Violation(rule, offending)` ONLY in enforce mode (caught by 25-06 per-symbol try/except → `PipelineRun.stats.failed_symbols`). Predicates: `predicate_rsi_anomaly` (RSI > 99.5, `dq/schemas/indicators.py`), `predicate_gap_30pct` (close-to-close > 30%, sorts by date defensively, drops first-bar NaN), `predicate_missing_rows_20pct` (frame-level signal when `1 - actual/expected > 0.20`). Wired into `AnalysisService.analyze_technical_single` after `compute_indicators`; `Tier2Violation` re-raised, all other exceptions logged as `dq.tier2.predicate_error` (defense-in-depth Rule 2 fix — buggy advisory check must never break analysis for healthy symbols). `metrics.iter_tracked_collectors(name)` + `_TRACKED_REGISTRIES: WeakSet[CollectorRegistry]` populated by `init_metrics(target)` so the dispatcher increments collectors on EVERY tracked registry (Rule 1 fix — required for the SC #4 RED test which uses `reg = CollectorRegistry(); init_metrics(reg)` and asserts `reg.get_sample_value(...) >= 1`). `docs/runbook/dq-tier2-promotion.md` (117 lines, replaces Wave 0 placeholder): Promotion Criteria (5-point gate: ≥14d advisory, <5% violation rate, no FP review, exemplar present, AutomationService digest reviewed), Procedure (Prometheus query → env flag flip → restart → verify → audit-log), Rollback (>30% symbol-failure trigger), Per-Rule Status Table. `test_runbooks.py` upgraded existence-only → content-required (asserts `Promotion Criteria`, `Rollback`, `Per-Rule Status Table`, `DQ_TIER2_`, `shadow`, `enforce`, `14-day`). New `test_sc4_tier2_emits_metric_no_block` — verbatim ROADMAP SC #4 closure. 6/6 dispatch tests + 1/1 runbook content test GREEN; 83/83 in test_services + test_dq + test_docs (no regressions). 25-08 RED tests (`test_health_data_freshness.py` × 3) remain RED — next plan.
Last activity: 2026-04-29 — 25-07 Wave 5: 3 commits (`98e7e3c`, `f09c9bb`, `42ae535`), 9 files modified, 6/6 dispatch + 1/1 runbook GREEN, no regressions. All commits carry Copilot co-author trailer. ROADMAP SC #4 verbatim ✅ closed.
