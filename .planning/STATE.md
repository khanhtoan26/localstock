...
status: Phase 26 Caching IN PROGRESS — Wave 1 partial: 26-01 complete (CACHE-04 ✅, ROADMAP SC #3 ✅ closed). 26-02/03/04 spawn next (Wave 2 parallel). 26-05/06 follow.
stopped_at: Completed 26-01-PLAN.md (Wave 1 — cache core + single-flight + Wave-0 fixtures + canonical cache_compute_total)
last_updated: "2026-04-29T09:00:00.000Z"
last_activity: 2026-04-29 — 26-01 complete (CACHE-04 ✅, SC #3 ✅). Built `localstock.cache` package: InstrumentedTTLCache subclass (Q-2 popitem hook → cache_evictions_total{cache_name, reason} with reason='expire'|'evict' via _in_expire flag), per-key asyncio.Lock single-flight via WeakValueDictionary (P-2 lazy alloc inside coroutine + P-3 strong-local-ref pattern), eager-purge invalidate_namespace, cache_outcome_var ContextVar, public get_or_compute(namespace, key, compute_fn, ttl=None) with cache_compute_total.inc INSIDE lock AFTER double-check BEFORE await (P-9 / SC #3 gate). Canonical `cache_compute_total{cache_name}` Counter declared in observability/metrics.py (B1 — 26-01 owns; 26-02 will add disjoint cache_prewarm_errors_total). Added get_metrics() public accessor (Rule 3 fix — plan referenced it but only _DEFAULT_METRICS existed). Wave-0 shared fixtures landed in tests/conftest.py: db_session (transactional rollback, requires_pg-gated, lifted from test_quarantine_repo.py:22-50) + async_client (httpx.AsyncClient + ASGITransport(create_app())); test_quarantine_repo.py refactored — file-local db_session removed, symbol cleanup moved to test-local autouse _clean_quarantine_test_rows fixture wrapping the shared one. cachetools>=5,<6 added; 6 cache_* settings appended to Settings (cache_ranking_ttl=86400, cache_market_ttl=3600, cache_indicators_ttl=3600, cache_indicators_maxsize=600, cache_janitor_interval=60, cache_latest_run_id_ttl=5). 7/7 cache tests GREEN including test_concurrent_cold_key_single_compute (100 racers, compute_calls==1, counter delta==1 — verbatim SC #3 closure). Full suite 553 passed (only pre-existing Phase-24 migration downgrade fails — out of scope). uvx ruff clean. 3 commits with Copilot co-author trailer (c15fbcc test fixtures, 3b17482 RED, a3d1493 GREEN).
progress:
  total_phases: 8
  completed_phases: 4
  total_plans: 30
