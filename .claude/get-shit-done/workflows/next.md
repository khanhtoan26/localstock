...
```bash
# Get state snapshot
node "/home/toanak/playground/localstock/.claude/get-shit-done/bin/gsd-tools.cjs" state json 2>/dev/null || echo "{}"
```
...
```bash
node "/home/toanak/playground/localstock/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs: defer incomplete Phase {src} items to backlog"
```
